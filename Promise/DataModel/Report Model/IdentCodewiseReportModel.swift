...
import Foundation
struct IdentCodewiseReportModel : Codable {
    let count : Int?
    let next : String?
    let previous : String?
    let results : [IdentCodeResults]?
    ...
    init(from decoder: Decoder) throws {
        ...
        results = try values.decodeIfPresent([IdentCodeResults].self, forKey: .results)
    }
}
struct IdentCodeResults : Codable {
    let vendor : String?
    let po : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        released_pieces = try values.decodeIfPresent(Int.self, forKey: .released_pieces)
