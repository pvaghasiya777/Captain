...
class PurchaseOrderVC: UIViewController {
    ...
    //Pagination View
     @IBOutlet var viewPagination: UIView!
    ...
    func InitlizationView() {
        self.tbl_PurchaseOrder.register(UINib(nibName: "PurchaseOrderCell", bundle: nil), forCellReuseIdentifier: "PurchaseOrderCell")
        ...
        Utils.EnableTextField(textFields: [txt_Number,txt_Vendor,txt_Project,txt_POType,txt_Address,txt_ShipperDetails,txt_Date,txt_DeliveryDate,txt_Currency,txt_WeightUOM,txt_Dimension,txt_Volume])
    ...
    func setData() {
        ...
        if Arr_PurchaseOrder.count != 0 {
            self.btn_isActive.setBackgroundImage(UIImage(named: (Arr_PurchaseOrder[0].is_active!) ? "ic_check" : "ic_Uncheck"), for: .normal)
            ...
            self.lbl_ShowPagination.text = "Showing \(startIndex) to \(((startIndex + 10) > Arr_PurchaseOrder[0].purchase_lines!.count) ? Arr_PurchaseOrder[0].purchase_lines!.count : (startIndex + 10)) of \(Arr_PurchaseOrder[0].purchase_lines!.count) results"
        }
    }
    ...
    @objc func btn_NextClick(_ sender: UIButton) {
        if startIndex + 10 > Arr_PurchaseOrder[0].purchase_lines!.count - 1 {
...
extension PurchaseOrderVC : UITableViewDataSource , UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return min(10,((Arr_PurchaseOrder.count == 0) ? Arr_PurchaseOrder.count : Arr_PurchaseOrder[0].purchase_lines!.count - startIndex))
    }
